...
saved_starts_file = 'New_ALLLOWfould3_q_20260801_174820';  % Name of the saved starting-point file.
...
problem_library = [
    ...
    make_problem("bental4", "bental4new.nl",   @generate_bental4_start_new, "low", -450)
    make_problem("bental5", "bental5_new.nl",   @generate_bental5_start_new, "low", -3500)
...
for problem_id = 1:numel(problems)
    ...
    fprintf('\n=== QN-SQP-quadprog | %s | %s | mode=%s | runs=%d ===\n', ...
    ...
    if save_starts && ~use_saved_starts
        starts_file = char("New_ALLLOW" + cfg.name + "_q_" + timestamp + ".mat");
...
for k = 1:max_iter
    ...
    d0 = zeros(n,1);
    [d, ~, exitflag, ~, lambda] = quadprog( ...
        H, g, Aqp, bqp, [], [], lb_d, ub_d, d0, options);
    ...
    if exitflag <= 0
        ...
        for rest_iter = 1:max_rest_iter
            ...
            if lp1_feasible
                ...
                [dmain, ~, exitflag, ~, lambda] = quadprog( ...
                    H_rest_main, g_rest, Acanon_rest, -ccanon_rest, [], [], ...
                    lb_d_rest, ub_d_rest, dlp, options);
                if exitflag > 0
    ...
    if ~accepted && ~restoration_phase && h_trial > 0
        ...
        while true
            ...
            [d_soc, ~, socflag, ~, lambda_soc] = quadprog( ...
                H, g, Asoc, bsoc, [], [], ...
                lb_d, ub_d, d_soc_prev, options);
...
restfun = struct('H', Hrest, 'f', frest);
drest0 = zeros(n,1);

[drest, restfval, restflag, restoutput] = quadprog( ...
    Hrest, frest, Arest, brest, [], [], ...
    lb_d_rest, ub_d_rest, drest0, options);
...
[d_soc, ~, socflag, ~] = quadprog( ...
    restfun.H, restfun.f, Arest_soc, brest_soc, [], [], ...
    lb_d_rest, ub_d_rest, d_soc_prev, options);
