...
xlabel('Iteration k');
ylabel('Value');
legend('\Delta_k', '||d_k||_\infty', '||d_k||_\infty / \Delta_k', ...
    'Location', 'best');
grid on;
export_validation_figure(fig3, out_dir, sprintf('%s_qnsqp_trust_region_activity', file_prefix));
...
if isfield(cfg, 'x_star')
    ...
    fig4 = figure('Name', 'QN-SQP exact-solution error');
    semilogy(k_err, err_plot, '-o', 'LineWidth', 1.2, 'MarkerSize', 4);
    xlabel('Iteration k');
    ylabel('||x_k - x_*||_2');
    grid on;
    export_validation_figure(fig4, out_dir, sprintf('%s_qnsqp_error_history', file_prefix));
...
fprintf('%s\n', fullfile(out_dir, sprintf('%s_qnsqp_trust_region_activity.pdf', file_prefix)));
if isfield(cfg, 'x_star')
    fprintf('%s\n', fullfile(out_dir, sprintf('%s_qnsqp_error_history.png', file_prefix)));
    fprintf('%s\n', fullfile(out_dir, sprintf('%s_qnsqp_error_history.pdf', file_prefix)));
end
