...
run_timer = tic;
[result, log] = solve_pool_once(prob, n, m, bl, bu, cl, cu, x_start, cfg);
result.solve_time = toc(run_timer);

fprintf('\n=== exact-Hessian SQP validation | %s | %s | mode=%s ===\n', ...
...
fprintf('rest records  : %d\n', result.num_restoration);
fprintf('main SOC      : %d\n', result.num_main_soc);
...
save(fullfile(out_dir, sprintf('%s_exact_sqp_validation.mat', file_prefix)), ...
    'result', 'log', 'x_start', 'cfg');

plot_haverly_validation(log, result, cfg);


function plot_haverly_validation(log, result, cfg)
if isempty(log.iter)
...
entered_restoration = [iter.entered_restoration];
...
file_prefix = lower(regexprep(char(cfg.name), '[^A-Za-z0-9]+', '_'));
method_label = 'exact-Hessian SQP';

fig1 = figure('Name', sprintf('%s feasibility history', char(cfg.name)));
semilogy(k, h_plot, '-o', 'LineWidth', 1.2, 'MarkerSize', 4);
hold on;
rest_k = k(entered_restoration);
for i = 1:numel(rest_k)
    xline(rest_k(i), '--r', 'LineWidth', 0.9);
end
xlabel('Iteration k');
ylabel('Constraint violation h_k');
title(sprintf('%s %s: feasibility history', char(cfg.name), method_label));
grid on;
saveas(fig1, fullfile(out_dir, sprintf('%s_exact_sqp_feasibility_history.png', file_prefix)));
...
fig2 = figure('Name', sprintf('%s filter trajectory', char(cfg.name)));
...
grid on;
saveas(fig2, fullfile(out_dir, sprintf('%s_exact_sqp_filter_trajectory.png', file_prefix)));

fig2b = figure('Name', sprintf('%s representative filter marks', char(cfg.name)));
plot(h_acc_plot, f_acc, 'o', 'Color', [0 0.4470 0.7410], ...
    'LineWidth', 1.2, 'MarkerSize', 4);
hold on;
set(gca, 'XScale', 'log');
x_max = max(h_acc_plot);
y_max = max(f_acc);
filter_show_idx = [5 14 18 20 28];
filter_show_idx = filter_show_idx(filter_show_idx <= numel(h_acc_plot));
for jj = 1:numel(filter_show_idx)
    i = filter_show_idx(jj);
    plot([h_acc_plot(i), x_max], [f_acc(i), f_acc(i)], ...
        'k-', 'LineWidth', 0.9);
    plot([h_acc_plot(i), h_acc_plot(i)], [f_acc(i), y_max], ...
        'k-', 'LineWidth', 0.9);
end
plot(h_acc_plot(filter_show_idx), f_acc(filter_show_idx), ...
    'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 4);
xlabel('Constraint violation h');
ylabel('Objective value f');
title(sprintf('%s %s: selected dominated regions', char(cfg.name), method_label));
grid on;
saveas(fig2b, fullfile(out_dir, sprintf('%s_exact_sqp_filter_marks.png', file_prefix)));

fig3 = figure('Name', sprintf('%s trust-region activity', char(cfg.name)));
semilogy(k, max(rho, 1e-16), '-o', 'LineWidth', 1.2, 'MarkerSize', 4);
hold on;
semilogy(k, max(norm_d, 1e-16), '-s', 'LineWidth', 1.2, 'MarkerSize', 4);
semilogy(k, max(tr_activity, 1e-16), '-^', 'LineWidth', 1.2, 'MarkerSize', 4);
xlabel('Iteration k');
ylabel('Value');
legend('\rho_k', '||d_k||_\infty', '||d_k||_\infty / \rho_k', ...
    'Location', 'best');
title(sprintf('%s %s: trust-region activity', char(cfg.name), method_label));
grid on;
saveas(fig3, fullfile(out_dir, sprintf('%s_exact_sqp_trust_region_activity.png', file_prefix)));

fprintf('\n=== validation plots saved ===\n');
fprintf('%s\n', fullfile(out_dir, sprintf('%s_exact_sqp_feasibility_history.png', file_prefix)));
fprintf('%s\n', fullfile(out_dir, sprintf('%s_exact_sqp_filter_trajectory.png', file_prefix)));
fprintf('%s\n', fullfile(out_dir, sprintf('%s_exact_sqp_filter_marks.png', file_prefix)));
fprintf('%s\n', fullfile(out_dir, sprintf('%s_exact_sqp_trust_region_activity.png', file_prefix)));
...
if result.num_restoration == 0
    fprintf('\nNo restoration phase was entered in this validation run.\n');
else
    fprintf('\nRestoration was entered at outer iteration(s): ');
    fprintf('%d ', unique(rest_k));
    fprintf('\n');
    plot_restoration_progress(log, out_dir, cfg);
end
end


function plot_restoration_progress(log, out_dir, cfg)
if ~isfield(log, 'restoration') || isempty(log.restoration)
    return
end

rest = log.restoration;
outer_ids = unique([rest.outer_iter]);
file_prefix = lower(regexprep(char(cfg.name), '[^A-Za-z0-9]+', '_'));

for jj = 1:numel(outer_ids)
    outer_id = outer_ids(jj);
    idx = [rest.outer_iter] == outer_id;
    R = rest(idx);

    r = [R.rest_iter];
    h_trial = [R.h_l1_trial];
    h_entry = R(1).h_l1_now;
    accepted_rest = [R.accepted_rest];

    valid_trial = isfinite(h_trial);
    accepted_idx = accepted_rest & valid_trial;
    rejected_idx = ~accepted_rest & valid_trial;

    fig = figure('Name', sprintf('%s restoration progress outer %d', char(cfg.name), outer_id));
    semilogy(0, max(h_entry, 1e-16), 'bd', ...
        'MarkerFaceColor', 'b', 'MarkerSize', 5);
    hold on;
    path_r = [0, r(accepted_idx)];
    path_h = [h_entry, h_trial(accepted_idx)];
    if numel(path_r) > 1
        semilogy(path_r, max(path_h, 1e-16), ...
            'k-', 'LineWidth', 0.9);
    end
    if any(accepted_idx)
        semilogy(r(accepted_idx), max(h_trial(accepted_idx), 1e-16), ...
            'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 4);
    end
    if any(rejected_idx)
        semilogy(r(rejected_idx), max(h_trial(rejected_idx), 1e-16), ...
            'rx', 'MarkerSize', 6, 'LineWidth', 1.2);
    end

    xlabel('Restoration inner iteration');
    ylabel('L1 constraint violation');
    title(sprintf('%s restoration L1 progress at outer iteration %d', char(cfg.name), outer_id));
    legend('Restoration entry', 'Accepted restoration path', ...
        'Accepted trial', 'Rejected trial', 'Location', 'best');
    grid on;

    filename = sprintf('%s_exact_sqp_restoration_outer_%d.png', file_prefix, outer_id);
