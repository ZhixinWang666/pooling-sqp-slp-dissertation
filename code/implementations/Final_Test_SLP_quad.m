...
for problem_id = 1:numel(problems)
    ...
    fprintf('\n=== SLP quadprog | %s | %s | mode=%s | runs=%d ===\n', ...
    ...
    for run_id = 1:num_runs_this
        ...
        try
            ...
            [result, ~] = solve_slp_quad_once(prob, n, m, bl, bu, cl, cu, x_start, cfg);
        catch ME
    ...
    if save_starts && ~use_saved_starts
        starts_file = char("SLP_quad_starts_" + cfg.name + "_" + timestamp + ".mat");
...
function [result, log] = solve_slp_quad_once(prob, n, m, bl, bu, cl, cu, x, cfg)
...
for k = 1:max_iter
    ...
    Hqp = zeros(n);
    d0_qp = zeros(n,1);
    [d, ~, exitflag, ~, lambda] = quadprog_silent( ...
        Hqp, g(:), Aqp, bqp, [], [], lb_d, ub_d, d0_qp, qp_options);
