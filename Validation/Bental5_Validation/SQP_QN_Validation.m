...
save(fullfile(out_dir, sprintf('%s_qnsqp_validation.mat', file_prefix)), ...
...
fig1 = figure('Name', 'QN-SQP feasibility history');
...
grid on;
saveas(fig1, fullfile(out_dir, sprintf('%s_qnsqp_feasibility_history.png', file_prefix)));
...
fig2 = figure('Name', 'QN-SQP filter trajectory');
...
grid on;
saveas(fig2, fullfile(out_dir, sprintf('%s_qnsqp_filter_trajectory.png', file_prefix)));

fig3 = figure('Name', 'QN-SQP trust-region activity');
...
grid on;
saveas(fig3, fullfile(out_dir, sprintf('%s_qnsqp_trust_region_activity.png', file_prefix)));

fprintf('\n=== validation plots saved ===\n');
fprintf('%s\n', fullfile(out_dir, sprintf('%s_qnsqp_feasibility_history.png', file_prefix)));
fprintf('%s\n', fullfile(out_dir, sprintf('%s_qnsqp_filter_trajectory.png', file_prefix)));
fprintf('%s\n', fullfile(out_dir, sprintf('%s_qnsqp_trust_region_activity.png', file_prefix)));
...
if result.num_restoration == 0
    fprintf('\nNo restoration phase was entered in this QN-SQP run.\n');
else
...
for jj = 1:numel(outer_ids)
    ...
    fig = figure('Name', sprintf('%s QN-SQP restoration progress outer %d', char(cfg.name), outer_id));
    ...
    title(sprintf('%s QN-SQP restoration L1 progress at outer iteration %d', char(cfg.name), outer_id));
    legend('Restoration entry', 'Accepted restoration path', ...
        'Accepted trial', 'Rejected trial', 'Location', 'best');
    grid on;

    filename = sprintf('%s_qnsqp_restoration_outer_%d.png', file_prefix, outer_id);
...
for k = 1:max_iter
    ...
    if accepted
        ...
        if ~restoration_phase && isfield(lambda,'ineqlin') && length(lambda.ineqlin) >= m
            lambda_new = canonical_lambda_to_original(lambda.ineqlin, cl, cu, m);
        end

        if ~restoration_phase
            gradL_old = g + J' * lambda_before;

            g_new = eval_obj_grad(prob, x_trial);
            J_new = eval_jac_val(prob, 1:m, x_trial);
            J_new = J_new';

            gradL_new = g_new + J_new' * lambda_new;

            s_bfgs = x_trial - x_current;
            y_bfgs = gradL_new - gradL_old;

            B = damped_bfgs_update(B, s_bfgs, y_bfgs);
        end
...
function Bnew = damped_bfgs_update(B, s, y)
B = 0.5 * (B + B');
...
Bnew = B - (Bs * Bs') / sBs + (r * r') / sr;
Bnew = 0.5 * (Bnew + Bnew');

minEig = min(eig(Bnew));
if minEig < 1e-8
    Bnew = Bnew + (1e-8 - minEig) * eye(size(Bnew,1));
end
